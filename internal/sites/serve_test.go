...
func setupStaticHandler(t *testing.T) (*StaticHandler, storage.Store) {
	...
	handler := NewStaticHandler(cfg, store, cache, nil)
...
func TestStaticHandlerServeHTTP(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			host:       "demo.localhost:8443",
			path:       "/%2e%2e/x",
			wantStatus: http.StatusBadRequest, // httptest decodes %2e%2e to .. in URL.Path
		},
		{
...
func TestStaticHandlerReadAuthorizer(t *testing.T) {
	tmp := t.TempDir()
	store, err := storage.NewLocal(tmp)
	if err != nil {
		t.Fatal(err)
	...
	deny := func(ctx context.Context, user *auth.User, site string) error {
		return errors.New("denied")
	}
	denyHandler := NewStaticHandler(cfg, store, cache, deny)
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	req.Host = "demo.localhost:8443"
	w := httptest.NewRecorder()
	denyHandler.ServeHTTP(w, req)
	if w.Code != http.StatusForbidden {
		t.Fatalf("deny authorizer status = %d, want 403", w.Code)
	}

	allowHandler := NewStaticHandler(cfg, store, cache, nil)
	req = httptest.NewRequest(http.MethodGet, "/", nil)
	req.Host = "demo.localhost:8443"
	w = httptest.NewRecorder()
	allowHandler.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("nil authorizer status = %d, want 200", w.Code)
	}
}
