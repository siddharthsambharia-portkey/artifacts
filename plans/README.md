...
- ~~**CLI deploy bypasses the server entirely**~~ (resolved 2026-06-17, issue 006 / ADR 0005): the local `internal/cli/deploy.go` path still exists for Operators on the server host, but remote Builders/CI now use `artifact deploy --remote --token` (or `ARTIFACT_TOKEN`), which POSTs to `/api/v1/deploy` as a first-class **service account**. Auth is a sha256-hashed Bearer token (`artifact token create`) that short-circuits the SSO redirect via the `auth.TokenAuthenticator` decorator; deploys are attributed to the service account's own email. MCP is the remaining consumer (can reuse the same `--remote`/token client). See `docs/adr/0005-programmatic-deploy-tokens.md`.
